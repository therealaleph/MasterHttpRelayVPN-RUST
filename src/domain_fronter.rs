...
impl DomainFronter {
    ...
    pub async fn tunnel_batch_request_with_timeout(
        ...
        // Skip h2 for tunnel batches (#1040). Batched ops are already
        // coalesced into one HTTP request so h2 multiplexing adds no
        // benefit; the h1 pool path is simpler and avoids h2-specific
        // overhead (ready timeout, NonRetryable errors, concurrent
        // stream contention with long-poll batches). The caller's
        // `batch_timeout` still flows through to the h1 header-read
        // deadline below — pipelined seq's longer
        // `max(configured, PIPELINED_BATCH_TIMEOUT_FLOOR)` budget is
        // honored there so a legitimate server-side
        // `SEQ_WAIT_TIMEOUT + LONGPOLL_DEADLINE` wait doesn't fire a
        // spurious transport timeout.
        let mut entry = self.acquire().await?;
